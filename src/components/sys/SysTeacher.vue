      ...
      <el-row>   
        <el-col :span="6">
          <el-form-item label="职称" prop="title">
            <el-select v-model="searchData.title" placeholder="请选择">
              <el-option
        ...
        <el-col :span="6">
          <el-form-item label="入职年份" prop="entryTime">
            <el-date-picker
              v-model="searchData.entryTime"
              type="year"
              placeholder="选择年">
            </el-date-picker>
      ...
      <div style="margin-right: 20px">
        <el-form :model="updateData" :rules="rules" ref="updateData" label-width="100px">
          ...
          <el-row>
            <el-col :span="10" :offset="0">
              <el-form-item label="职称" prop="title">
                <el-select v-model="updateData.title" placeholder="请选择">
                  <el-option
                    v-for="item in titles"
                    :key="item.desc"
                    :label="item.desc"
            ...
            <el-col :span="10" :offset="1">
              <el-form-item label="入职年份" prop="entryTime">
                <el-input   v-model="updateData.entryTime" disabled></el-input>
              </el-form-item>
            </el-col>            
          </el-row>
          <el-row>
      ...
      <div style="margin-right: 20px">
        <el-form :model="addData" :rules="rules" ref="addData" label-width="100px">
          ...
          <el-row>
            ...
            <el-col :span="10" :offset="0">
              <el-form-item label="职称" prop="title">
                <el-select v-model="addData.title" placeholder="请选择">
                  <el-option
                    v-for="item in titles"
                    :key="item.desc"
                    :label="item.desc"
      ...
      <el-form :model="form">
        <el-form-item>
          <el-upload class="upload-file"
            drag
            ref="uploadExcel"
            action="http://114.116.194.209:8902/sys/tea/import"
...
export default {
  ...
  data() {
    return {
      ...
      Title:"修改教师", //"修改"对话框的标题
      rules:{ //表单验证
        name:[{ required:true,message:'姓名不能为空',trigger:'blur'}],
        number:[{required:true,message:'工号不能为空',trigger:'blur'},
        { pattern:/^[0-9]*$/,message:'工号输入格式错误，应为8位数字',trigger:'blur'},
        { min:5,max:5,message:'工号输入长度错误，应为8位数字',trigger:'blur'}],
        school:[{required:true,message:'学院不能为空',trigger:'blue'}],
        title:[{required:true,message:'职称不能为空',trigger:'blue'}],
  ...
  methods: {
    ...
    Delete(row) {
      console.log(row);
      this.$axios.delete('/sys/tea/delete', {	
        params: {	// 请求参数拼接在url上
          id: row.id
        }
      }).then(res => {
    ...
    showAllUserInfo(current, size) {
      ...
      this.$axios.post('/sys/tea/getTeacherList',this.searchData, {
        headers: {
    ...
    //获取所有学院
    getTitles(){
      this.$axios.get('/sys/tea/getTitles', {
      // params:{            
      // },
      headers: {
        "Content-Type": "application/json;charset=utf-8"
    ...
    Update(row){ //点击"修改"按钮弹出修改对话框
      ...
      self.titleValue = row.titleValue;
    },
    handlerAddOk(){ //修改对话框上的"确定"按钮
      let self = this;
      this.$refs["updateData"].validate(valid =>{
        if (valid){
          this.$axios.put('/sys/tea/update', self.updateData, {
            headers: {
              "Content-Type": "application/json;charset=utf-8"
            },
            withCredentials: true
    ...
    addStudent(){ //修改对话框上的"确定"按钮
      ...
      this.$refs["addData"].validate(valid =>{
        if(valid){
          let self = this;
          this.$axios.post('/sys/tea/add', self.addData, {
            headers: {
              "Content-Type": "application/json;charset=utf-8"
            },
            withCredentials: true
     ...
     batchDelete(){ //"批量删除"按钮绑定的函数
      ...
      if(self.multipleSelection.length!=0){
        self.$confirm("确定删除吗？","操作提示",{
          ...
          console.log(sid);
          this.$axios.delete('/sys/tea/batchDelete', {	
            params: {	// 请求参数拼接在url上
              sid: sid
            }
          }).then(res => {
